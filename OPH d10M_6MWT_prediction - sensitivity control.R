...
## purpose of analysis: sensitivity analysis for delta-6MWT predictors; repeating the main analysis for the control group
...
# Excluding SIPScore (n=76, 78,35%), AESI (n=55, 56,70%), CSI (n = 54, 55,67%), CTGDiagnostic (n=53, 54,64%)
...
write.csv(t_selected_pred[order(-t_selected_pred$Mean),], "OPH_6MWT_senscontrol_VIPs.csv", row.names = TRUE)
write.csv(t_selected_pred[order(-t_selected_pred$Mean),], "OPH_6MWT_senscontrol_VIPs.xlsx", row.names = TRUE)
