...
# restore dataframe - 2 outliers removed
bdf <- cdf
cdf <- NULL
...
bdf <- bdf[!is.na(bdf$ddm1a),] # Note: 110 cases remain (11 cases removed)

# Exclude all patients that have max baseline DM1-Activ-C scores [4 patients removed - 106 remain]
...
write.csv(na_count[order(-na_count$percentage),], "C:/Users/Daniel.vanAs/Documents/OPTIMISTIC Post-Hoc Paper R Scripts/Results/OPH_Missing_basline_values.csv", row.names = TRUE)
na_count[order(-na_count$percentage),]


# variables with > 25% missing values are removed
# Excluding CSI(n=62, 58.5%), AESI (n=62, 58,5%), CTGDiagnostic (n=58, 54,7%)
